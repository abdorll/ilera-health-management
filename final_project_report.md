...
│   │  7 API endpoints  │   │  16 API endpoints│   │  8 API endpoints │        │
│   └────────┬─────────┘   └────────┬─────────┘   └────────┬────────┘        │
...
│   │                                                                   │      │
│   │  ┌──────────────────────────────────────────────────────────┐    │      │
│   │  │  CORS: 3 origins (Patient, Admin, Doctor frontends)      │    │      │
...
│   │  │   (2 eps) │  │  (14 ops)   │  │ isPatientAuthenticated│      │      │
│   │  │ /user     │  │ appointCtrl  │  │ isDoctorAuthenticated │      │      │
│   │  │  (16 eps) │  │  (10 ops)   │  │ isPrincipalAdmin      │      │      │
│   │  │ /appoint  │  │ messageCtrl  │  │ errorMiddleware       │      │      │
│   │  │  (12 eps) │  │  (2 ops)    │  │ catchAsyncErrors      │      │      │
│   │  └──────────┘  └──────────────┘  └───────────────────────┘      │      │
│   │                                                                   │      │
│   │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌────────────┐      │      │
...
│   │  │ adminTkn │  │  salt:10)│  │  upload/ │  │  Welcome   │      │      │
│   │  │ patntTkn │  │          │  │  destroy)│  │  emails +  │      │      │
│   │  │ doctrTkn)│  └──────────┘  └──────────┘  │  change    │      │      │
│   │  └──────────┘                               │  alerts    │      │      │
│   │                                              └────────────┘      │      │
│   │  ┌──────────────────────────────────────────────────────────┐    │      │
│   │  │  Middleware Stack: cookie-parser · express.json ·        │    │      │
│   │  │  express.urlencoded · express-fileupload (tempFiles)     │    │      │
│   │  └──────────────────────────────────────────────────────────┘    │      │
│   └───────────────────────────────────────────────────────────────────┘      │
│                             │ Mongoose ODM                                    │
└─────────────────────────────┼─────────────────────────────────────────────────┘
...
- **Model Layer**: Mongoose schemas (`userSchema.js`, `appointmentSchema.js`, `messageSchema.js`) define the data structure and validation rules. The User model includes instance methods for password hashing (`pre-save` hook with bcrypt, salt factor 10), password comparison (`comparePassword`), and JWT generation (`generateJsonWebToken`). The Appointment model uses `timestamps: true` for automatic `createdAt`/`updatedAt` tracking and references the User model via `patientId` and `doctorId` foreign keys.
- **Controller Layer**: Controller files contain the business logic for handling HTTP requests. `userController.js` handles 14 operations (register, login, 3 logouts, add doctor/admin, get doctors/admins, get user details, edit/delete doctor, edit/delete admin). `appointmentController.js` handles 10 operations (post, get my, get all, assign, get by dept, doctor assigned, doctor unassigned, self-assign, respond, mark completed, update status, delete). `messageController.js` handles 2 operations (send, get all). Email dispatch logic (Nodemailer) is invoked before user creation to prevent orphan records.
...
│ (React.js)   │  │  │  Layer   │  │                                  │
│ adminToken   │  │  │          │  │  userController (14 operations)  │
└──────────────┘  │  │ /user/*  │  │  ├─ patientRegister              │
                  │  │ /appoint │  │  ├─ login (role-based)           │
┌──────────────┐  │  │  ment/*  │  │  ├─ addNewAdmin (+email)         │
│ Doctor SPA   │──┘  │ /message │  │  ├─ addNewDoctor (+email+avatar) │
│ (React.js)   │     │  /*      │  │  ├─ getAllDoctors / getAllAdmins  │
│ doctorToken  │     └────┬─────┘  │  ├─ editDoctor / editAdmin       │
└──────────────┘          │        │  ├─ deleteDoctor / deleteAdmin    │
                          ▼        │  ├─ logoutAdmin/Patient/Doctor    │
                     ┌──────────┐  │  └─ getUserDetails                │
                     │Middleware│  │                                    │
                     │  Stack   │  │  appointmentController (10 ops)   │
                     │          │  │  ├─ postAppointment               │
                     ...
                     │ ├──────┤ │  │  └─ deleteAppointment             │
                     │ │Prncpl│ │  │                                    │
                     │ │Admin │ │  │  messageController (2 operations)  │
                     │ └──────┘ │  │  ├─ sendMessage (public)           │
...
#### 2.4.1.1 Appointment Status State Machine
...
**Figure 2.3.1**: Appointment Status State Machine
...
```
                            ...
                            │                                                          │
  ┌──────────┐              │   ┌──────────────────────────────────┐                   │
  │  Visitor │──────────────┼──→│ View Hospital Info / Departments │                   │
  │ (Unauth) │              │   └──────────────────────────────────┘                   │
  │          │              │   ┌──────────────────────────────────┐                   │
...
|-------|-------------|----------------|
| **Visitor** | Unauthenticated user browsing the patient portal; can view hospital info and send contact messages | None required |
| **Patient** | Registered user who books appointments and tracks their status | `patientToken` cookie via JWT |
| **Principal Admin** | Root administrator (`admin@ilera.com`) with full CRUD over doctors and admins | `adminToken` cookie + `isPrincipalAdmin` middleware |
...
STATE MACHINE:
  ...
  [Patient Books] → PENDING
              ...
              ┌─────────┴──────────┐
              │                    │
    [Admin Assigns Doctor]   [Doctor Self-Assigns]
              │                    │
              ▼                    │
          ASSIGNED                 │
          │       │                │
...
ALTERNATIVE PATH (Doctor self-assignment):
1. Patient submits appointment → Status = "Pending"
2. Doctor views unassigned appointments in their department
3. Doctor self-assigns → Status = "Accepted" (skips "Assigned" state)
4. Doctor marks completed → Status = "Completed", hasVisited = true
```
...
| `nodemailer` | ^8.0.1 | Email sending via Gmail SMTP (doctor/admin onboarding, email change alerts) |
...
| `FRONTEND_DOCTOR` | Doctor Portal URL (for CORS) | `http://localhost:5176` |
| `CLOUDINARY_CLOUD_NAME` | Cloudinary cloud identifier | (from Cloudinary dashboard) |
| `CLOUDINARY_API_KEY` | Cloudinary API key | (from Cloudinary dashboard) |
| `CLOUDINARY_API_SECRET` | Cloudinary API secret | (from Cloudinary dashboard) |
| `SMTP_HOST` | SMTP server hostname | `smtp.gmail.com` |
| `SMTP_PORT` | SMTP server port | `465` |
| `SMTP_EMAIL` | Gmail address for sending emails | (Gmail address) |
| `SMTP_PASSWORD` | Gmail app password for SMTP | (App password) |
...
| Status | Admin Dashboard Label | Doctor Dashboard Label | Patient Dashboard Label |
|--------|----------------------|----------------------|------------------------|
| Pending | ⏳ Awaiting Doctor Assignment | — | Pending |
| Assigned | 📋 Doctor Assigned — Awaiting Response | 📋 Awaiting Your Response | Doctor Assigned |
| Accepted | ✅ Doctor Confirmed | ✅ You Accepted — Active | Doctor Accepted |
| Rejected | ❌ Doctor Declined — Needs Reassignment | — | Being Reassigned |
| Completed | 🏁 Visit Completed | 🏁 Visit Completed | Completed |
